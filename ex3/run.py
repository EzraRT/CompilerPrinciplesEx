...
from PyQt5.QtWidgets import QApplication, QWidget, QFileDialog, QAbstractItemView, QTableWidgetItem, QHeaderView,     QMessageBox
...
class MyWin(QWidget, Ui_LR1):
    ...
    def handle_analy(self):
        ...
        if len(self.inputStr) == 0:
            QMessageBox.warning(self, "错误", "输入为空！")
        elif len(LR1.Grammer) == 0:
            QMessageBox.warning(self, "错误", "请先导入文法！")
        else:
            self.child.show()
            self.child.processTable.verticalHeader().hide()
            self.child.processTable.setColumnCount(5)
            self.child.processTable.setHorizontalHeaderLabels(
                ["步骤", "状态栈", "符号栈", "输入串", "动作说明"])
    ...
    def totalControl(self):
        ...
        while True:
            ...
            self.child.processTable.setItem(
                step, 0, QTableWidgetItem(str(step)))
            self.child.processTable.setItem(
                step, 1, QTableWidgetItem(','.join(stack_status)))
            self.child.processTable.setItem(
                step, 2, QTableWidgetItem(''.join(stack_char)))
            self.child.processTable.setItem(
                step, 3, QTableWidgetItem(self.inputStr[cur:]))
            if ACTION[top].get(a) is None:
                info = "ACTION[" + top + ',' + a + ']未定义，分析失败'
                self.child.processTable.setItem(
                    step, 4, QTableWidgetItem(info))
                break
                ...
                #移进
                info = "ACTION[" + top + ',' + a + ']=' + \
                    ACTION[top][a]+',状态'+ACTION[top][a][1:]+'入栈'
                self.child.processTable.setItem(
                    step, 4, QTableWidgetItem(info))
                ...
                len_beta = len(right)
                info = "r"+beta_id+':'+left+'->'+right + \
                    '归约，GOTO('+stack_status[-1-len_beta]+','+left+')=' + \
                    GOTO[stack_status[-1-len_beta]][left]+'入栈'
                self.child.processTable.setItem(
                    step, 4, QTableWidgetItem(info))
                while(len_beta > 0):
                    stack_status.pop()
